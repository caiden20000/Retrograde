...
export function runTradeForSystem(system: System, revolutions: number): System {
  ...
  for (let currentRevolution = 0; currentRevolution <= revolutions; currentRevolution++) {
    for (const station of newSystem) {
      ...
      if (shortages.length > 0) {
        ...
        const itemQuantity = Trd.getItemBaseQuantity(station.tradeInventory, itemType) * SHORTAGE_THRESHOLD;
        const supplier = getNearestSupplier(system, station, itemType, itemQuantity);
        if (supplier == null) break;
        const newStations = tradeBetween(station, supplier, itemType, itemQuantity);
        newSystem = updateStationInSystem(newSystem, station, newStations.requester);
        newSystem = updateStationInSystem(newSystem, supplier, newStations.supplier);
      } else {
        // If no shortage, random trade to random station.
        const supplier = getRandomNeighborInRange(system, station, RANDOM_TRADE_RANGE);
...
function getShortagesFromStation(station: Station): ItemType[] {
  ...
  for (const itemType of allItemTypes) {
    const baseQuantity = Trd.getItemBaseQuantity(station.tradeInventory, itemType);
...
function sortSystemByDistance(system: System, station: Station) {
  const distFromRequester = (x: Station) => getDistance(station.position, x.position);
  const systemSortedByDistance = cloneSystem(system).sort((a, b) => distFromRequester(a) - distFromRequester(b));
...
function getNearestSupplier(
  ...
  const systemSortedByDistance = sortSystemByDistance(system, requestingStation);
...
function getRandomNeighborInRange(system: System, station: Station, range: number) {
  const stationsWithinRange = system.filter((stn) => stationDistance(station, stn) <= range);
...
function tradeBetween(
  ...
  // if (supply < itemQuantity) {
  //   console.log(`Error! ${supply} < ${itemQuantity}`);
  // }
  // if (isNaN(itemQuantity)) {
  //   console.log(`Error! itemQuantity WAS ALREADY NaNd`);
  // }
  const returnValue = {
    requester: set(requester, {
      tradeInventory: Trd.addItemCount(requester.tradeInventory, itemType, itemQuantity),
    }),
    supplier: set(supplier, {
      tradeInventory: Trd.addItemCount(supplier.tradeInventory, itemType, -itemQuantity),
    }),
  };
  // const newSupply = Trd.getItemCount(
  //   returnValue.supplier.tradeInventory,
  //   itemType
  // );
  // if (isNaN(newSupply)) {
  //   console.log(`Error! ${newSupply} NaNd`);
  // }
  return returnValue;
}
...
function hasTradeableAmount(station: Station, itemType: ItemType, itemQuantity: number): boolean {
  const stationQuantity = Trd.getItemCount(station.tradeInventory, itemType);
  const stationBaseQuantity = Trd.getItemBaseQuantity(station.tradeInventory, itemType);
  const minimumQuantityToTrade = itemQuantity + stationBaseQuantity / SHORTAGE_THRESHOLD;
  return stationQuantity > minimumQuantityToTrade;
}

function randomTradeBetween(requester: Station, supplier: Station): { requester: Station; supplier: Station } | null {
  ...
  for (const itemType of allItemTypes) {
    const supplierQuantity = Trd.getItemCount(supplier.tradeInventory, itemType);
    if (supplierQuantity > 0 && requesterItems.some((item) => itemType === item)) possibleItems.push(itemType);
  }

  // Pick a random amount that won't cause a shortage
  for (const itemType of shuffle(possibleItems)) {
    const supplierQuantity = Trd.getItemCount(supplier.tradeInventory, itemType);
    const supplierBaseQuantity = Trd.getItemBaseQuantity(supplier.tradeInventory, itemType);
    if (supplierBaseQuantity === undefined) continue;
    const tradeableQuantity = supplierQuantity - supplierBaseQuantity * SHORTAGE_THRESHOLD;
    if (tradeableQuantity <= 0) continue;
    const maximum = Math.min(tradeableQuantity, supplierBaseQuantity * MAX_TRADE);
    const minimum = Math.min(tradeableQuantity, supplierBaseQuantity * MIN_TRADE);
